...
public:
	 ...
	 *  value() is the estimated value of V(s'')
	 *
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				// compute an esitmated value of a move
				// r + sum_s''(P(s, a, s'')*V(s''))
				int num = 0;
				float esti_Vnext = 0;
				for (int i = 0; i < 16; i++){
					...
					if (next.at(i) == 0) {
						...
						next.set(i, 1);
						esti_Vnext += 0.9 * estimate(next);

						next.set(i, 2);
						esti_Vnext += 0.1 * estimate(next);
			        }
				}

				esti_Vnext /= num;
				move->set_value(move->reward() + esti_Vnext);
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for (path.pop_back() /* terminal state */; path.size(); path.pop_back()) {
			...
			debug << "update error = " << error << " for state" << std::endl << move.before_state();
...
int main(int argc, const char* argv[]) {
	...
	*/
	...
	// open a csv file to write episode scores
	//std::ofstream out;
	//out.open("episode.csv");
	...
	for (size_t n = 1; n <= total; n++) {
		...
		// write the current episode score into csv file
		//out << score << std::endl;
		path.clear();
